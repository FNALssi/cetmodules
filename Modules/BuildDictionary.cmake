...
set(_cet_build_dictionary_flags NOP NO_CHECK_CLASS_VERSION NO_EXPORT
  ...
  USE_PROJECT_NAME VERBOSE)

set(_cet_build_dictionary_one_arg_options CLASSES_DEF_XML CLASSES_H
  DICT_NAME_VAR EXPORT_SET GENERATED_SOURCE_FILENAME
  LIB_TARGET LIB_TARGET_VAR MODULEMAP_INSTALL_DIR VERBOSITY)

set(_cet_build_dictionary_list_options CCV_ENVIRONMENT COMPILE_FLAGS
  DICTIONARY_LIBRARIES REQUIRED_DICTIONARIES SOURCE)
...
function(build_dictionary)
  ...
  if (BD_UNPARSED_ARGUMENTS)
	  message(FATAL_ERROR  "build_dictionary: non-option arguments not permitted: ${BD_UNPARSED_ARGUMENTS}")
  endif()
  if (DEFINED BD_VERBOSITY AND NOT BD_VERBOSITY MATCHES "^[0-9]+$")
    message(FATAL_ERROR "build_dictionary: invalid option VERBOSITY ${BD_VERBOSITY}")
  endif()
  ...
  if (BD_SOURCE)
    ...
    if (n_xml_files GREATER 1)
      message(FATAL_ERROR "build_dictionary: SOURCE should specify at most ONE classes_def.xml file")
    endif()
  ...
  cet_process_liblist(dictionary_liblist ${BD_LIB_TARGET} PRIVATE ${BD_DICTIONARY_LIBRARIES})
  set(cml_args)
  ...
  if (BD_COMPILE_FLAGS)
    set_source_files_properties(${BD_GENERATED_SOURCE_FILENAME}
...
function(_generate_dictionary)
  ...
  set(target_includes "$<$<BOOL:${tmp_includes}>:-I$<JOIN:${tmp_includes},$<SEMICOLON>-I>>")
  get_property(dir_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  ...
  if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/module.modulemap")
    ...
    cet_copy("${CMAKE_CURRENT_SOURCE_DIR}/module.modulemap"
    ...
    if (NOT ROOT_VERSION GREATER_EQUAL 6.10.04 AND
      ...
      # Header line and OS X lib name fixing in .rootmap only necessary for older ROOT6.
      add_custom_command(TARGET ${BD_LIB_TARGET} POST_BUILD
  ...
  list(APPEND ROOTCLING_FLAGS
    -s "$<TARGET_LINKER_FILE:${BD_LIB_TARGET}>")
  ...
  if (ROOTCLING_MODULEMAP AND NOT BD_NO_INSTALL)
    ...
    if (NOT BD_MODULEMAP_INSTALL_DIR)
      set(BD_MODULEMAP_INSTALL_DIR "${${CETMODULES_CURRENT_PROJECT_NAME}_INCLUDE_DIR}")
    endif()
    string(MAKE_C_IDENTIFIER "BuildDictionary_modulemap_${BD_MODULEMAP_INSTALL_DIR}" MODULEMAP_CONSOLIDATION_TARGET)
    cet_package_path(current_subdir)
    string(MAKE_C_IDENTIFIER "${current_subdir}" subdir_identifier)
    cet_copy(${ROOTCLING_MODULEMAP} NO_ALL
      NAME ${subdir_identifier}.modulemap
      TARGET_VAR modulemap_target
      DESTINATION "${PROJECT_BINARY_DIR}/${MODULEMAP_CONSOLIDATION_TARGET}")
    if (NOT TARGET ${MODULEMAP_CONSOLIDATION_TARGET})
      add_custom_target(${MODULEMAP_CONSOLIDATION_TARGET} ALL)
      # Two-tier target ensures all modulemap files are copied to the
      # temporary directory before we attempt to concatenate them all.
      add_custom_command(OUTPUT "${PROJECT_BINARY_DIR}/${MODULEMAP_CONSOLIDATION_TARGET}.modulemap"
        COMMAND ${CMAKE_COMMAND} -E cat --
        ${PROJECT_BINARY_DIR}/${MODULEMAP_CONSOLIDATION_TARGET}/*.modulemap > ${MODULEMAP_CONSOLIDATION_TARGET}.modulemap
        WORKING_DIRECTORY "${PROJECT_BINARY_DIR}"
        COMMAND_EXPAND_LISTS
      )
      add_custom_target(${MODULEMAP_CONSOLIDATION_TARGET}_CAT ALL
        DEPENDS "${PROJECT_BINARY_DIR}/${MODULEMAP_CONSOLIDATION_TARGET}.modulemap"
      )
      add_dependencies(${MODULEMAP_CONSOLIDATION_TARGET}_CAT ${MODULEMAP_CONSOLIDATION_TARGET})
      install(FILES "${PROJECT_BINARY_DIR}/${MODULEMAP_CONSOLIDATION_TARGET}.modulemap"
        DESTINATION "${BD_MODULEMAP_INSTALL_DIR}" RENAME module.modulemap)
    endif()
