...
#    ENABLE_ASSERTS
#
#      Enable asserts regardless of CMAKE_BUILD_TYPE (default is to disable
#      asserts for all but Debug).
#
#    EXTRA_FLAGS <flags>
#    EXTRA_C_FLAGS <flags> (DEPRECATED)
#    EXTRA_CXX_FLAGS <flags> (DEPRECATED)
#    EXTRA_DEFINITIONS <flags>
#
#      These list parameters will append the specified flags via
#      add_compile_options() or
#      add_compile_definitions(). EXTRA_<lang>_FLAGS options are
#      deprecated: use EXTRA_FLAGS with generator expressions instead
#      e.g. EXTRA_FLAGS $<$<COMPILE_LANGUAGE:<lang>[,<lang>]>:<flags>>
#
#    NO_UNDEFINED
#
...
#    WERROR
#
#      All warnings are flagged as errors unless countermanded with
#      -Werror=no-<diag> or -Wno-<diag>.
#
####################################
# cet_enable_asserts()
#
...
# Notes:
#
#   * This function will only remove explicit flags from CMake's global
#     variables (CMAKE_CXX_COMPILER_FLAGS, etc.), and definitions (-D,
#     -U) with scoped remove_definitions() commands. It cannot preclude
#     the same options being added back in some other way, nor can it
#     remove flags set in properties, nor configuration- or
#     language-specific options added via generator expressions.
#
...
function(cet_report_compiler_flags)
  cmake_parse_arguments(PARSE_ARGV 0 CRCF "" "REPORT_LEVEL" "")
  if (NOT CRCF_REPORT_LEVEL)
    set(CRCF_REPORT_LEVEL STATUS)
  endif()
  set(gen_exps)
  cet_package_path(current_subdir HUMAN_READABLE)
  message(${CRCF_REPORT_LEVEL} "Global CMAKE_<lang>_FLAGS* variables in the current scope"
  ...
  list(TRANSFORM vars
    REPLACE "${flags_regex}" "\\1"
    OUTPUT_VARIABLE langs)
  ...
  foreach (var IN LISTS vars)
    ...
    message(${CRCF_REPORT_LEVEL} "  ${var} = ${${var}}")
  endforeach()
  message(${CRCF_REPORT_LEVEL} "These may be overridden by:")
  foreach(prop IN ITEMS COMPILE_DEFINITIONS COMPILE_OPTIONS
      INCLUDE_DIRECTORIES LINK_OPTIONS LINK_DIRECTORIES)
    get_property(items DIRECTORY PROPERTY ${prop})
    if (items)
      if (items MATCHES [[$<]])
        list(APPEND gen_exps ${prop})
      endif()
      message(${CRCF_REPORT_LEVEL} "  * property ${prop} for the current scope:")
      foreach(item IN LISTS items)
        message(${CRCF_REPORT_LEVEL} "      ${item}")
      endforeach()
    endif()
  endforeach()
  message(${CRCF_REPORT_LEVEL} "  * properties modified at lower-level directories")
  message(${CRCF_REPORT_LEVEL} "  * target- and source-specific directories.")
  if (gen_exps)
    list(JOIN gen_exps ", " gen_exps_string)
    message(${CRCF_REPORT_LEVEL} "some variables or properties (${gen_exps_string})"
      " contain generator expressions (\$<...>) and will be resolved later.")
      message(${CRCF_REPORT_LEVEL} "build with generator verbosity to see actual definitions and"
...
function(_parse_flags_options)
  ...
  foreach (var IN ITEMS C CXX KEYWORDS_MISSING_VALUES QUIET LANGUAGES UNPARSED_ARGUMENTS)
    if (DEFINED CSCF_${var})
      set(CSCF_${var} "${CSCF_${var}}" PARENT_SCOPE)
    endif()
  endforeach()
endfunction()
...
macro(cet_set_compiler_flags)
  ...
  if (diag_idx GREATER -1)
    ...
    if (diag_idx GREATER 0) # At least CAUTIOUS
      add_compile_options("SHELL:$<$<COMPILE_LANGUAGE:C,CXX>:-Wall -Werror=return-type>")
      if (diag_idx GREATER 1) # At least VIGILANT
        add_compile_options(-Wextra -Wno-long-long -Winit-self)
        add_compile_options("SHELL:$<$<AND:$<COMPILE_LANGUAGE:C,CXX>,$<OR:$<COMPILE_LANG_AND_ID:$<COMPILE_LANGUAGE>,Clang,AppleClang>,$<AND:$<COMPILE_LANG_AND_ID:$<COMPILE_LANGUAGE>,GNU>,$<VERSION_GREATER_EQUAL:$<$<COMPILE_LANGUAGE>_COMPILER_VERSION>,4.7.0>>>>:-Wno-unused-local-typedefs>")
        add_compile_options("SHELL:$<$<COMPILE_LANGUAGE:CXX>:-Wdelete-non-virtual-dtor>") # C++ only
        add_compile_options("SHELL:$<$<AND:$<COMPILE_LANGUAGE:CXX>,$<OR:$<CXX_COMPILER_ID:Clang,AppleClang>,$<AND:$<CXX_COMPILER_ID:GNU>,$<VERSION_GREATER_EQUAL:$<CXX_COMPILER_VERSION>,4.7.0>>>>:-Woverloaded-virtual -Wnon-virtual-dtor>") # C++ only
