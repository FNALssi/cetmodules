...
sub find_args_for {
  ...
  my $results;
  while (
     defined(my $kw_idx = find_keyword($call_info, $wanted_keyword, $offset)))
  {
    $results->{$kw_idx} = [];
...
sub is_quoted {
  ...
  if (((ref $args[0]) // q()) eq 'HASH') { # ($call_info, $idx_idx)
    ...
    $result =
      ($search_result->{qs} eq q(])) ? "[$search_result->{qmarker}\E[" :
      $search_result->{q};
  } else {
...
sub remove_args_at {
  ...
  while (@arg_indexes) {
    ...
    while (defined $last_idx_idx and
           ($arg_indexes[0] // $_NO_MATCH) == $last_idx_idx + 1) {
      ++$n_items;
  ...
  while (1) {
    ...
    ($line, $chunk_start_line, $line_no, $current_linepos,
     $expect_whitespace, $in_quote)
      = @{
      _extract_args_from_string($cmakelists,
                                {
                                   line              => $line,
...
EOF
    ...
    defined $value_index and
      $call_info->{chunk_locations}->{$value_index} = $chunk_start_line;
...
EOF
      $error_message = sprintf($msg_fmt,
                               ...
                               $chunk_start_line,
                               $current_linepos,
                               join(q(), reconstitute_code($call_info), $line
                                   ));
    } else {
      $error_message =
...
sub _prepare_cml_io {
  ...
  if (ref $cmakelists) {
    ...
    $cml_in->opened() and
      $cml_in->seek(0, Fcntl::SEEK_SET) or
      error_exit(
       "input filehandle provided for $cmakelists must be open and rewindable"
      );
  } else {
    debug("opening CMake file $cmakelists");
    $cml_in = IO::File->new("$cmakelists", "<") or
      error_exit("unable to open $cmakelists for read");
  }
  if ($options->{output}) {
    if (ref $options->{output}) {
      ref $options->{output} eq "ARRAY" or
        error_exit(
...
sub _process_pending_comments {
  ...
  debug(sprintf('processing comments from %s:%s%s',
                $cmakelists,
                $comments->{start_line},
...
sub _process_cml_lines {
  ...
  my ($cmakelists, $cml_in, $cml_out, $pending_comments) =
    @{$cml_data}{qw(cmakelists cml_in cml_out pending_comments)}
    ; # Convenience.
  ...
  if ($line =~ m&\A\s*[#].*\z&msx) { # Full-line comment.
    ...
    exists $pending_comments->{start_line} or
      $pending_comments->{start_line} = $line_no;
    return $line_no;
  }
...
EOF
    ...
    if ($call_info->{post} =~ m&[)]\s*[#]&msx or
        scalar @{ $call_info->{comment_indexes} // [] } and
        exists $options->{comment_handler}) {
      debug(sprintf(<<"EOF", $call_info->{name}));
