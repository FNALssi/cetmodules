sub old_process_cmakelists {
  ...
  while (scalar @$buffer) {
    ...
    if (not $cmr_done) {
      if ($line =~
m&^\s*(?i:cmake_minimum_required)\s*\(\s*VERSION\s+(?P<vmin>.*?)(?:\.\.\.(?P<vmax>.*?))?\s*(?P<fatal>FATAL_ERROR)?\s*\)(?P<post>.*)$&
        ) {
        my $cmr_info = {%+};
        if ($cmr_info->{vmin} and version_cmp($cmr_info->{vmin}, '3.19') < 0)
        {
          $cmr_info->{vmin} = '3.19';
          if ($cmr_info->{vmax} and
              version_cmp($cmr_info->{vmin}, $cmr_info->{vmax}) != -1) {
            $cmr_info->{policy} = $cmr_info->{vmax};
            undef $cmr_info->{vmax};
          }
        }
        $fh->printf(
"cmake_minimum_required(VERSION $cmr_info->{vmin}%s FATAL_ERROR)%s\n",
        ...
        arg_group: foreach my $arg (@$arg_group) {
            ...
            unless ($seen_arg) {
              ...
              if ($arg =~ m&^(v)?(?:\d+[_.]?)+&) {
                warning(
                  sprintf(
"unexpected project() argument $arg looks like a \%sversion without the required VERSION keyword: mitigating",
                    ($1 ? "UPS " : "")));
                $arg = to_cmake_version($arg);
                unshift @$arg_group, "VERSION";
                $VERSION_next = 1;
              } elsif (grep { $arg eq $_; }
                       qw(NONE CXX C Fortran CUDA ISPC OBJC OBJCXX ASM)) {
                verbose
"prefixing language $arg with LANGUAGES keyword in project().";
          ...
          } ## end arg_group: foreach my $arg (@$arg_group)
        } ## end all_args: foreach my $arg_group (@all_args)
        if ($pi->{version}) {
...
EOF
              ...
              $$seen_version =
                sprintf("\%s$piversion\%s\%s",
                        ($version_info->{q} // '') x 2,
                        $version_info->{nl} // '');
            } ## end if ($piversion and $version_info...)
          } else {
            if ($proj_info->{pre} =~ m&^(.*?)([ 	]*+\n?+)$&) {
              $proj_info->{pre} =
                sprintf("\%s\%s",
                        join(" ", $1, "VERSION", $piversion), $2 // '');
            }
          }
        } ## end if ($pi->{version})
        $fh->print("find_package(cetmodules $CETMODULES_VERSION REQUIRED)\n")
          unless $seen_find_package;
        $fh->print(
          $proj_info->{pre},
          join(
            "  ",
...
m&^\s*(?i:project)(?:\s*(?:#[^\n]*)\n?)*(?:\s*\((?:\s*(?:#[^\n]*)\n?)*)?$&s
    ...
    } ## end if (not $project_done)
    if ($line =~ m&\s+###\s+MIGRATE-NO-ACTION\b$&) {
      $fh->print($line);
      next;
    }
    if ($line =~
m&^\s*(?i:find_(?P<find>package|ups_product))\s*\(\s*(?P<pkg>cet(?:buildtools|modules))\s+(?P<minv>(?P<minvMajor>\d+)[^\s)]+)?&
      ) {
      if ($+{find} eq 'package' and $+{pkg} eq "cetmodules") {
        $line =~ m&^\s*find_package\s*\(\s*cetmodules(\s+[^1)]|\s*\))& or
          $line =~
s&^(\s*find_package\s*\(\s*cetmodules\s+)[^\s)]++(.*)$&${1}${CETMODULES_VERSION}${2}&;
        $seen_find_package = 1;
      } else {
    ...
    $line =~
s&^(\s*)cet_report_compiler_flags\s*\(\s*\)&${1}cet_report_compiler_flags(REPORT_THRESHOLD VERBOSE)&;
...
EOF
      warning($msg);
    } ## end if ($line =~ m&^\s*(?i:subdirs)\s*\($&))
    $line =~ m&^\s*(?i:include)\s*\(\s*UseCPack& and next;
    $fh->print($line);
  } ## end while (scalar @$buffer)
  if (scalar @shunt) {
    @$buffer = @shunt;
  } else {
  ...
  return ++$pass;
} ## end sub old_process_cmakelists
  ...
  while (my $line = <$in>) {
    if (not defined $seen_line1) {
      ...
      if ($line =~ m&^###\s+MIGRATE-NO-ACTION\b&) {
        info(
"upgrading $filepath -> $filepath.new SKIPPED due to MIGRATE-NO-ACTION in line 1"
        );
        $in->close();
        $out->close();
    ...
    while ($line =~ m&\$\{(?:\$\{product\}|\Q$pname\E)_([_\w]+)\}&g) {
      ...
      my ($var_stem) = map {
        ($dirkey_ish eq $_ or $dirkey_alt eq $_) ? var_stem_for_dirkey($_) :
          ();
      } keys %$PATHSPEC_INFO;
      $line =~
s&(\$\{(?:\$\{product\}|\Q$pname\E)_\Q$dirkey_ish\E\})&\${\${CETMODULES_CURRENT_PROJECT_NAME}_$var_stem\}&g
        and
        pos($line) = 0 and
        verbose("$1 -> \${\${CETMODULES_CURRENT_PROJECT_NAME}_$var_stem}");
    } ## end while ($line =~ m&\$\{(?:\$\{product\}|\Q$pname\E)_([_\w]+)\}&g)
    $line =~
s&\$\{gdml_install_dir\}&\${\${CETMODULES_CURRENT_PROJECT_NAME}_GDML_DIR}&g
      and
      verbose(
       '${gdml_install_dir} -> ${${CETMODULES_CURRENT_PROJECT_NAME}_GDML_DIR}'
      );
    flag_recommended($line, ": use add_subdirectory()")
      if ($line =~ m&^\s*(?i:subdirs)\s*\($&);
...
": declare exportable CMake module directories with cet_cmake_module_directories()"
    ...
    flag_recommended($line,
                ": use find_package() to handle external modules directories")
      if $line =~ m&\bCMAKE_MODULE_PATH\b.*\$ENV$&;
    $line =~ s&(\$\{product\}/\${version\}/)&&g and
      verbose("\${product}/\${version}/ -> \"\"");
    $line =~
s&(\$\{flavorqual(?:_dir)?\})&\${\${CETMODULES_CURRENT_PROJECT_NAME}_EXEC_PREFIX}&g
      and
      verbose(
             "$1 -> \"\${\${CETMODULES_CURRENT_PROJECT_NAME}_EXEC_PREFIX}\"");
    ...
    while (
      $line =~ m&\G.*?\b(?P<clause>find_package\b\s*\(\s*(?P<pkg>[^\s)"]+))&g)
    {
    ...
    flag_recommended($line,
                     ": use target_link_directories() with target semantics")
    ...
    flag_recommended($line,
                     ": use target_link_libraries() with target semantics")
    ...
    flag_recommended($line,
                 ": use cet_find_package() with target semantics for linking")
      if $line =~ m&\b(find_ups_product|(?:cet_)find_library)\b&;
    flag_recommended($line, ": use cet_test()") if $line =~ m&\b(add_test)\b&;
    flag_recommended(
      $line,
": use art_make_library(), art_dictionary(), simple_plugin() with explicit source lists"
      )
      if $line =~ m&\b(art_make\b)&;
    flag_recommended(
      $line,
": use cet_make_library(), build_dictionary(), basic_plugin() with explicit source lists"
      )
